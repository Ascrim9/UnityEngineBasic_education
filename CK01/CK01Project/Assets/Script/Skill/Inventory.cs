...
public class Inventory : Singleton<Inventory>
{
    ...
    private void Start()
    {
        ...
        equipmentSlot = equipSlotParent.GetComponentsInChildren<UI_EquipmentSlot>();
    ...
    {
        ItemData_Equipment curFlask = GetEquipment(EquipmentType.Flask);

        if (curFlask == null) return;

        bool canUseFlask = Time.time > lastTimeUsedFlask + curFlask.itemCooldown;

        if(canUseFlask)
        {
            flaskCooldown = curFlask.itemCooldown;
            curFlask.Effect(null);
    ...
    {
        ItemData_Equipment curArmor = GetEquipment(EquipmentType.Armor);
        if(Time.time > lastTimeUsedFlask + armorCooldown)
        {
            armorCooldown = curArmor.itemCooldown;
            lastTimeUsedFlask = Time.time;
            return true;
        }
